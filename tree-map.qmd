...
tree_shp <- st_read("data analysis/cleaned data/coe_tree_archive.shp",
                    quiet = TRUE) 
my_names <- read_csv("data analysis/cleaned data/faculty-tree-names.csv")
colnames(my_names) <- c("ID", "tree_name")

tree_shp <- merge(tree_shp, my_names, by="ID", all.x = T)
tree_shp <- tree_shp %>% 
  mutate(
    tree_name = ifelse(is.na(tree_name), "Unnamed", tree_name)
  )
